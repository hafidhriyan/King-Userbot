...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/ridho17-ind/King-Userbot.git")
...
async def check_alive():
    await bot.send_message(BOTLOG_CHATID, "**🐲Flicks-Userbot🐲 V.0.5.2** __Telah Diaktifkan Oleh {ALIVE_NAME}__")
    return
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query.startswith("@UserButt"):
                ...
                result = builder.article(
                    ...
                    f"\n\n◎› **King** {DEFAULTUSER}\n\n"
                    "◎› **Plugins :** `{}`\n◎› **Menu Plugins ↯** \n".format(
                        "**⚡Flicks-Userbot⚡**", len(dugmeler), ), buttons=buttons, link_preview=False, )
            elif query.startswith("tb_btn"):
                result = builder.article(
                    "Bantuan ⚡Flicks-Userbot⚡ ",
                    text="◎› Daftar Plugins",
                    buttons=[],
                    link_preview=True)
            else:
                result = builder.article(
                    "**⚡Flicks-Userbot⚡**",
                    text="""**Anda Bisa Membuat Flicks Userbot Anda Sendiri Dengan Cara:** [Tekan Disini](t.me/sadroomchat)""",
                    buttons=[
                        [
                            custom.Button.url(
                                "Flicks-Userbot",
